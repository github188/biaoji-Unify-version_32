...
int MeterReadRecogV2(HYLPAMR_IMAGES src,MeterDESCRIP Meterdescrip,HYAMR_Meter_Result *MeterResult){
	...
	if(Meterdescrip.MeterType==1){
		...
		//peizhifile="E:/biaoji.mat";
		//imgfile="F:/SF6表/001_1.png";
	}
	else{
		printf("未知仪表类型！！！\n");
		res=-1;
	...
	else
	{
		...
		IndexImage416.pixelArray.chunky.pPixel=(MByte *)calloc(IndexImage416.lHeight*IndexImage416.pixelArray.chunky.lLineBytes,sizeof(MByte));
		Index416Data=(MByte*)IndexImage416.pixelArray.chunky.pPixel;
		...
		//opencv  sf6刻度区域拉直打印
		/*MByte *indexData416; 
		IplImage *indeximg416=cvCreateImage(cvSize(IndexImage416.lWidth,IndexImage416.lHeight),IPL_DEPTH_8U,3);
		indexData416 = (MByte*)indeximg416->imageData;
		dstData = (MByte*)IndexImage416.pixelArray.chunky.pPixel;
		for(j=0;j<IndexImage416.lHeight;j++){
			for(i=0;i<3*IndexImage416.lWidth;i++){
				indexData416[i+j*indeximg416->widthStep]=dstData[i+j*IndexImage416.pixelArray.chunky.lLineBytes];
			}
		}
		cvSaveImage("D:\\indeximg416.bmp", indeximg416);
		cvReleaseImage(&indeximg416);*/
